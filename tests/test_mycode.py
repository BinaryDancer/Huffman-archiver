...
class Archiver:
    ...
    def compress(self, file_path, verbose=False) -> None:
        st_time = time()
        ...
        if len(self.bdict) == 0:
            ...
            bstr_out_data = "0" * 9 + "{:0>8}".format(bin(len(file_path.split(".")[-1]))[2:]) + bstr_file_type
            cnt_zeros = 8 - len(bstr_out_data) % 8
            bstr_out_data += "0" * cnt_zeros + \
                "{:0>8}".format(bin(cnt_zeros)[2:])
        ...
        huff_tree: Archiver.Node = self._create_huff_tree([(v, f) for v, f in self.bdict.items()])
        ...
        bstr_file_type = "".join(["{:0>8}".format(bin(ord(el))[2:]) for el in file_path.split(".")[-1]])
        bstr_out_data = bstr_huff_table + "0" * 9 + \
            "{:0>8}".format(bin(len(file_path.split(".")[-1]))[2:]) + bstr_file_type + bstr_encoded_data
        cnt_zeros = 8 - len(bstr_out_data) % 8
    ...
    def decompress(self, file_path, verbose: bool = False) -> None:
        st_time = time()
        ...
        output_path = ".".join(file_path.split(".")[:-1]) + "." + file_type
    ...
    def _parse_bdata(self, bdata: str, verbose: bool = False) -> tuple:
        ...
        while i < len(bdata):
            ...
            if code_len == 0:
                break
            mapping_dict[bdata[i: i+code_len]
                         ] = bdata[i+code_len: i+code_len+8]
            i += code_len + 8
